...
// Test 3.1: Cascade when primary provider fails
{
    ...
    try {
        ...
        globalThis.fetch = async (url, init = {}) => {
            ...
            if (urlStr.includes('api.openai.com')) {
                ...
                // Simulate OpenAI returning SSE stream
                const sseBody = [
                    'data: {"choices":[{"delta":{"content":"Paris is the "}}]}\n\n',
        ...
        assert.deepEqual(Array.from(new Set(attemptedProviders)), ['groq', 'openai'], 'Should attempt Groq first, then OpenAI on failure');
        assert.equal(streamedDeltas.join(''), 'Paris is the capital of France.', 'Should stream combined tokens from fallback provider');

        console.log('  [PASS] Cascade successfully fell back from failed Groq to OpenAI.');
    } finally {
...
// Test 3.2: Cascade when Groq and OpenAI fail -> Gemini succeeds
{
    ...
    try {
        ...
        assert.equal(result.ok, true, 'Cascade should succeed on Gemini when Groq & OpenAI fail');
        assert.equal(result.provider, 'gemini', `Expected provider 'gemini', got ${result.provider}`);
        assert.deepEqual(Array.from(new Set(attemptedProviders)), ['groq', 'openai', 'gemini'], 'Should cascade through Groq -> OpenAI -> Gemini');
        assert.equal(streamedDeltas.join(''), 'Photosynthesis converts light energy into glucose.');

        console.log('  [PASS] Cascade successfully fell back through Groq -> OpenAI -> Gemini.');
    } finally {
        globalThis.fetch = originalFetch;
        process.env.GROQ_API_KEY = originalGroqKey;
        process.env.OPENAI_API_KEY = originalOpenAIKey;
...
// Test 3.3: User-selected OpenAI model priority -> attempts OpenAI first
{
    const originalFetch = globalThis.fetch;
    const originalGroqKey = process.env.GROQ_API_KEY;
    const originalOpenAIKey = process.env.OPENAI_API_KEY;
    ...
    try {
        ...
        globalThis.fetch = async (url) => {
            ...
            if (urlStr.includes('api.openai.com')) {
                attemptedProviders.push('openai');
                const sseBody = 'data: {"choices":[{"delta":{"content":"GPT-4o response."}}]}\n\ndata: [DONE]\n\n';
                return new Response(new ReadableStream({
                    start(controller) {
                        controller.enqueue(new TextEncoder().encode(sseBody));
                        controller.close();
        ...
        assert.deepEqual(attemptedProviders, ['openai'], 'Should attempt OpenAI first when userSelectedModel is gpt-4o');
        assert.equal(streamedDeltas.join(''), 'GPT-4o response.');

        console.log('  [PASS] User-selected OpenAI model priority successfully executed.');
    } finally {
        globalThis.fetch = originalFetch;
        process.env.GROQ_API_KEY = originalGroqKey;
        process.env.OPENAI_API_KEY = originalOpenAIKey;
