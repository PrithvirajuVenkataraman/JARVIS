...
// 4.2: Verify cascade fall-through from Groq to Gemini on failure
await asyncCheck('4.2 Multi-provider stream cascade executes fallback from Groq to Gemini on error', async () => {
    ...
    try {
        ...
        globalThis.fetch = async (url, init = {}) => {
            ...
            if (urlStr.includes('generativelanguage.googleapis.com')) {
                ...
                const sseBody = [
                    'data: {"candidates":[{"content":{"parts":[{"text":"Canberra is the capital."}]}}]}\n\n'
                ].join('');
        ...
        assert.deepEqual(Array.from(new Set(attemptedProviders)), ['groq', 'gemini'], 'Should attempt Groq first, then fallback to Gemini');
