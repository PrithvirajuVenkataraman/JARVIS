...
assert.equal(autoCandidates[0], 'llama-3.3-70b-versatile', 'Auto mode must prioritize verified active Llama 3.3 70B on Groq first');
assert.equal(autoCandidates[1], 'llama-3.1-8b-instant', 'Auto mode must prioritize Llama 3.1 8B Instant second');
assert.equal(autoCandidates[2], 'qwen-2.5-coder-32b', 'Auto mode must prioritize Qwen 2.5 Coder 32B third');
console.log('  [PASS] 1.1 Auto mode accurately orders Llama 3.3 70B -> Llama 3.1 8B -> Qwen 2.5 Coder -> GPT-OSS');
