...
const ev1 = tracker.track('query_dispatched', { query: 'who is the cm of tamil nadu' });
assert.ok(ev1.id.startsWith('tr_'));
assert.equal(ev1.name, 'query_dispatched');
assert.equal(ev1.payload.query, 'who is the cm of tamil nadu');
...
unsubscribe();
tracker.track('model_synthesis', { model: 'gpt-4o' });
...
const trustedSources = [
    { title: 'Tamil Nadu Gov Portal', url: 'https://www.tn.gov.in/ministers', sourceType: 'official_source' },
    { title: 'Wikipedia Assembly Info', url: 'https://en.wikipedia.org/wiki/Tamil_Nadu' }
];
...
assert.ok(resGrounded.verifiedDomains.includes('tn.gov.in'));
