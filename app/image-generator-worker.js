    ...
    if (action === 'generate') {
        ...
        try {
            ...
            if (hasGpu) {
                ...
                // Graceful Zero-Cost Fast Cloud Fallback
                self.postMessage({ action: 'progress', id, phase: 'fallback', percent: 25, message: 'Connecting to fast AI engine...' });
                ...
                self.postMessage({ action: 'progress', id, phase: 'download', percent: 60, message: 'Rendering AI image with Turbo...' });

                // Active abort timeout of 20 seconds to prevent hanging
                ...
                let res = null;
                try {
                    res = await fetchWithTimeout(fallbackUrl, 20000);
