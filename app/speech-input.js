/**
 * Unified Enterprise Voice-to-Text (VTT / Dictation) Controller.
 ...
 * Features: Real-time interim results, spoken punctuation, acronym capitalization, zero-leak audio streams.
 */
...
export function evaluateTurnCompleteness(transcript = '', options = {}) {
    ...
    if (TRAILING_CONNECTORS_REGEX.test(text) || INCOMPLETE_INTERROGATIVE_REGEX.test(text)) {
        return {
            isComplete: false,
...
export function createSpeechInputController(options = {}) {
    ...
    function initBrowserRecognition() {
        ...
        try {
            ...
            r.onresult = event => {
                ...
                const candidateText = (currentFinal || currentInterim).trim().toLowerCase();
                const activeSpoken = (typeof globalThis.getActiveSpokenText === 'function' ? globalThis.getActiveSpokenText() : '').toLowerCase();

                // Acoustic Echo Guard: If speech synthesis is active and the incoming words match the active spoken text,
                // ignore it to prevent speaker-to-mic feedback loops.
                if (isSpeaking && activeSpoken && candidateText) {
                    const candWords = candidateText.replace(/[^\p{L}\p{N}\s]/gu, ' ').split(/\s+/).filter(w => w.length > 1);
    ...
    async function toggleDictation() {
        if (typeof window !== 'undefined' && window.isSecureContext === false && window.location?.hostname !== 'localhost' && window.location?.hostname !== '127.0.0.1') {
            callbacks.onError('Microphone access requires HTTPS or localhost.');
            return false;
        }
        if (mode === 'dictation') {
            stop();
...
export function installSpeechInputUI(options = {}) {
    ...
    globalThis.toggleConverseMode = async () => false;
