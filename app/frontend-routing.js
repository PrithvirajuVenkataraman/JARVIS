...
const IMAGE_POSITIVE_PATTERNS = [
    ...
    /^(?:(?:hey|hi|hello)\s+)?(?:(?:bot|jarvis|ai|assistant)\s*,?\s+)?(?:can\s+you\s+(?:please\s+)?|could\s+you\s+(?:please\s+)?|please\s+)?(?:draw|paint|sketch)\s+(?:me\s+)?(?:an?\s+)(?:[a-zA-Z])/i,
...
export function extractImagePrompt(text) {
    ...
    const prefixRegex = /^(?:(?:hey|hi|hello)\s+)?(?:(?:bot|jarvis|ai|assistant)\s*,?\s+)?(?:can\s+you\s+(?:please\s+)?|could\s+you\s+(?:please\s+)?|please\s+|i\s+(?:want|need)\s+(?:you\s+to\s+)?(?:to\s+)?|help\s+me\s+)?(?:generate|create|render|make|draw|paint|sketch|produce)\s+(?:me\s+)?(?:an?\s+)?(?:ai\s+)?(?:image|picture|photo|photograph|drawing|painting|illustration|artwork|wallpaper|portrait|graphic|visual)(?:\s+(?:of|showing|depicting|with|for|about|featuring)|\s*[:-])\s*/i;
    if (prefixRegex.test(raw)) {
        return raw.replace(prefixRegex, '').trim();
    }

    const drawMeRegex = /^(?:(?:hey|hi|hello)\s+)?(?:(?:bot|jarvis|ai|assistant)\s*,?\s+)?(?:can\s+you\s+(?:please\s+)?|could\s+you\s+(?:please\s+)?|please\s+)?(?:draw|paint|sketch)\s+(?:me\s+)?/i;
    if (drawMeRegex.test(raw)) {
        return raw.replace(drawMeRegex, '').trim();
    }

    const photoOfRegex = /^(?:(?:hey|hi|hello)\s+)?(?:(?:bot|jarvis|ai|assistant)\s*,?\s+)?(?:can\s+you\s+(?:please\s+)?|could\s+you\s+(?:please\s+)?|please\s+)?(?:i\s+(?:want|need)\s+)?(?:an?\s+)?(?:image|picture|photo|drawing|illustration|artwork)\s+(?:of|showing|depicting|with)\s+/i;
