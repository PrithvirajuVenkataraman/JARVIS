...
async function getInstantFactHelper() {
    ...
    const STREAM_MODEL_FETCH_TIMEOUT_MS = 7_000;
    ...
    function getPreferredGroqCandidates(configuredModel = '', { preferSpeed = false, tier = 'standard', userSelectedModel = null, isReasoningQuery = false } = {}) {
        ...
        let mappedGroq = '';
        if (USER_SELECTABLE_MODELS.has(userSelected) && !userSelected.toLowerCase().includes('llama')) {
...
const edgeResponseCache = new EdgeSemanticLruCache();
    ...
    async function runSingleQualityModel(prompt, maxTokens, timeoutMs) {
        const groqApiKey = process.env.GROQ_API_KEY || process.env.GROQ_KEY;
        if (groqApiKey) {
            const model = String(process.env.GROQ_QUALITY_MODEL || 'qwen-2.5-coder-32b').trim();
            const response = await fetchWithTimeoutRetry('https://api.groq.com/openai/v1/chat/completions', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
