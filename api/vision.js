...
function buildVisionPrompt(userPrompt, task) {
    ...
    return [
        isTextTask
            ? 'You are a production-grade OCR and document vision engine.'
            : 'You are a production-grade scene understanding vision engine.',
        ...
        isTextTask
            ...
            : '- For scene tasks, identify the most prominent real-world subject first: animal, person, vehicle, product, food, or other object.',
        isSceneTask
        ...
        '  "objects": [',
        '    { "label": "car", "count": 1, "confidence": 0.88 }',
        '  ],',
        '  "people": [',
        ...
        isTextTask
            ...
            : '- For scene tasks: answer with the main visible subject first (animal/vehicle/person/object). Mention readable text only if useful secondary detail or if the user asked about text.'
    ].join('\n');
}
...
function isSceneVisionTask(task) {
    return task === 'animal_detect'
        || task === 'object_detect'
        || task === 'general_vision'
        || task === 'people_count';
}

function mergeSceneSubjects(objects = [], animals = [], people = []) {
    const merged = [];
    for (const animal of normalizeDetected(animals)) {
        merged.push({
            label: animal.label,
            count: animal.count,
            confidence: animal.confidence ?? 0.78
        });
    }
    for (const person of (Array.isArray(people) ? people : [])) {
        const count = Math.max(1, Number(person?.count || 1));
    ...
    for (const object of normalizeDetected(objects)) {
        merged.push(object);
    }
    return normalizeDetected(merged);
}

function formatDetectedSubjectsLine(subjects, fallbackLabel = 'items') {
    if (!Array.isArray(subjects) || !subjects.length) return '';
    return subjects
        .slice(0, 4)
        .map(item => `${String(item?.label || fallbackLabel)} x${Math.max(1, Number(item?.count || 1))}`)
        .join(', ');
}
...
function formatVisionResponse(data, task, userPrompt = '') {
    ...
    const sceneSubjects = mergeSceneSubjects(objects, animals, people);
    ...
    const textFocused = isTextFocusedVisionQuery(userPrompt);
    ...
    if (task === 'bill_summary') {
        ...
        return parts.join('\n\n') || conciseVisionFallback(directAnswer, summary, sceneSubjects);
    }
    ...
    if (task === 'animal_detect' && animals.length) {
        const animalLine = formatDetectedSubjectsLine(animals, 'animal');
        const answer = removeDetectedTextLead(compactSingleLine(directAnswer || summary));
        if (answer && !/^detected text:/i.test(answer)) {
            return cleanVisionDisplayText(answer);
        }
...
function buildObjectFirstVisionAnswer({ directAnswer, summary, objects, mainSubject, subjectType, conditionOrState, likelyReason, careOrSafetyNote, brand, model, modelEvidence, distinctiveFeatures, uncertainty, compactText, explainIntent, detailedIntent, includeText }) {
    ...
    const answer = removeDetectedTextLead(compactSingleLine(directAnswer || summary));
    const parts = [];
    // Drop weak brand/model guesses that have no evidence.
    const evidenceBacked = Array.isArray(modelEvidence) && modelEvidence.length > 0;
...
function pickTopObjectMeta(objects) {
    ...
    const skip = /^(text|words|lettering|logo|label|screen|display|brand|writing|document|paper|sign|poster|banner|wallpaper)$/i;
