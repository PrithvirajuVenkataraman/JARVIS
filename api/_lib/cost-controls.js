...
export function getCostControls() {
    return {
        ...
        defaultMaxTokens: clampInt(process.env.JARVIS_DEFAULT_MAX_TOKENS, 2200, 256, 8000),
        fastMaxTokens: clampInt(process.env.JARVIS_FAST_MAX_TOKENS, 900, 256, 4000),
        streamMaxTokens: clampInt(process.env.JARVIS_STREAM_MAX_TOKENS, 1800, 256, 6000)
    };
}
...
export function applyCostCapToLengthPolicy(lengthPolicy = {}, options = {}) {
    ...
    const maxTokens = clampInt(lengthPolicy?.maxTokens, cap, 256, 12000);
    return {
        ...lengthPolicy,
